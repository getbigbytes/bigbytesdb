...
echo 'Start bigbytes-query node-1'
nohup env RUST_BACKTRACE=1 target/${BUILD_PROFILE}/bigbytes-query -c scripts/ci/deploy/config/bigbytes-query-node-1.toml --internal-enable-sandbox-tenant >./.bigbytes/query-1.out 2>&1 &

echo "Waiting on node-1..."
python3 scripts/ci/wait_tcp.py --timeout 30 --port 9091

echo 'Start bigbytes-query node-2'
env "RUST_BACKTRACE=1" nohup target/${BUILD_PROFILE}/bigbytes-query -c scripts/ci/deploy/config/bigbytes-query-node-2.toml --internal-enable-sandbox-tenant >./.bigbytes/query-2.out 2>&1 &

echo "Waiting on node-2..."
python3 scripts/ci/wait_tcp.py --timeout 30 --port 9092

echo 'Start bigbytes-query node-3'
env "RUST_BACKTRACE=1" nohup target/${BUILD_PROFILE}/bigbytes-query -c scripts/ci/deploy/config/bigbytes-query-node-3.toml --internal-enable-sandbox-tenant >./.bigbytes/query-3.out 2>&1 &
