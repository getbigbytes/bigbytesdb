...
use super::v1::BigbytesQueryFlightService;
use crate::servers::Server as BigbytesQueryServer;
...
impl FlightService {
    pub fn create(config: InnerConfig) -> Result<Box<dyn BigbytesQueryServer>> {
        Ok(Box::new(Self {
            config,
    ...
    pub async fn start_with_incoming(&mut self, addr: SocketAddr) -> Result<()> {
        let flight_api_service = BigbytesQueryFlightService::create();
        let builder = Server::builder();
        let mut builder = if self.config.tls_rpc_server_enabled() {
            info!("bigbytesdb query tls rpc enabled");
            builder
...
impl BigbytesQueryServer for FlightService {
    #[async_backtrace::framed]
