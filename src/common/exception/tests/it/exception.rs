...
use bigbytes_common_exception::SerializedError;
use tonic::Code;
use tonic::Status;

#[test]
fn test_format_with_error_codes() {
    use bigbytes_common_exception::exception::*;
...
fn test_derive_from_std_error() {
    ...
    use bigbytes_common_exception::exception::ToErrorCode;

    let fmt_rst: std::result::Result<(), std::fmt::Error> = Err(std::fmt::Error {});

    let rst1: bigbytes_common_exception::exception::Result<()> =
    ...
    let rst2: bigbytes_common_exception::exception::Result<()> =
