    ...
    vec![
        ...
        (
            Box::new(ClientType::Ttc(
                "getbigbytes/ttc-rust:latest".to_string(),
                TTC_PORT_START,
            )),
            5,
        ),
        (
            Box::new(ClientType::Ttc(
                "ghcr.io/getbigbytes/ttc-go:latest".to_string(),
                TTC_PORT_START + 1,
            )),
            5,
        ),
    ...
    match client_type {
        ...
        ClientType::Hybird => {
            ...
            for (t, s) in ts.iter() {
                ...
                if acc >= r {
                    return create_bigbytes(t.as_ref(), filename).await;
                }
            }
            unreachable!()
        }
...
async fn run_suits(args: SqlLogicTestArgs, client_type: ClientType) -> Result<()> {
    ...
    if args.complete {
        for file in files {
            ...
            let mut runner =
                Runner::new(|| async { create_bigbytes(&client_type, &file_name).await });
            runner
                .update_test_file(
                    file.unwrap().path(),
                    col_separator,
...
async fn run_file_async(
    ...
    let mut runner = Runner::new(|| async { create_bigbytes(client_type, filename).await });
