...
function generate_keys() {
    # generate rsa key pairs for CA, bigbytesdb server and bigbytesdb client
    ...
    (cd client;  openssl pkcs12 -export -out client-identity.pfx -inkey pkcs8-client-key.pem -in client.pem -certfile ../ca/ca.pem -password pass:bigbytesdb)
}
